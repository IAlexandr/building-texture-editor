...
router.put('/:RegisterNo/:ID', function (req, res) {
  ...
  db.EditedBuilding.findOne(
    {'properties.ID': newBuilding.properties.ID }, (err, doc) => {
      ...
      if (doc) {
        db.EditedBuilding.update({'properties.ID': newBuilding.properties.ID }, newBuilding, {}, (err, numReplaced, upsert) => {
          if (err) {
            return res.status(500).json({errmessage: err.message});
          }
          const features = stroeniya_result_object_by_register_no[req.params.RegisterNo];
...
router.get('/:RegisterNo/:ID/:wallId', function (req, res) {
  ...
  let filePath = path.resolve(options.sourceDirPath, RegisterNo + '_' + ID + '_' + wallId + '.jpg');
  try {
    fs.statSync(filePath);
  } catch (err) {
    filePath = path.resolve(options.sourceDirPath, 'default.jpg');
  }
  const rs = fs.ReadStream(filePath);
  rs.pipe(res);
});

router.post('/:RegisterNo/:ID/:wallId', function (req, res) {
  const {RegisterNo, ID, wallId} = req.params;
  let filePath = path.resolve(options.sourceDirPath, RegisterNo + '_' + ID + '_' + wallId + '.jpg');
  try {
