...
import {rewriteToFile, prepSimplifyFeaturePoints, prepFloors} from './../modules/utils';
...
router.get('/buildings/simplify', (req, res) => {
  db.Building.find({}, (err, docs) => {
    ...
    async.eachLimit(features, 1, (feature, done) => {
      ...
      db.Building.update({_id: feature._id}, { $set: { geometry: feature.geometry }}, (err) => {
        return done(err);
      });
    }, (err) => {
      if (err) {
