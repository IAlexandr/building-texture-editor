...
export const actionTypes = {
  ...
  COPY_BUILDING_TEXTURE_COMPLETE: 'COPY_BUILDING_TEXTURE_COMPLETE',
  COPY_BUILDING_TEXTURE_FAILED: 'COPY_BUILDING_TEXTURE_FAILED',
  COPY_BUILDING_TEXTURE: 'COPY_BUILDING_TEXTURE',
...
export function getOriginTextures (registerNo, imagesInfo) {
  return dispatch => {
    dispatch(smessagesActions.openMessage('Пожалуйста подождите, выполняется подготовка изображений'));
    setTimeout(() => {
      const textures = [];
...
export function copyBuildingTextureComplete ({ copyInfo }) {
  return {
    type: actionTypes.COPY_BUILDING_TEXTURE_COMPLETE,
    copyInfo
  };
}

export function copyBuildingTextureFailed ({ copyInfo, err }) {
  return {
    type: actionTypes.COPY_BUILDING_TEXTURE_FAILED,
    err,
    copyInfo
  };
}

export function copyBuildingTexture (copyInfo) {
  return dispatch => {
    dispatch({
      type: actionTypes.COPY_BUILDING_TEXTURE,
      copyInfo
    });
    superagent.post('/api/buildings/copy')
      .accept('application/json')
      .send(copyInfo)
      .end((err) => {
        if (err) {
          dispatch(smessagesActions.openMessage('Ошибка копирования текстуры.'));
          dispatch(saveBuildingTexturesFailed({ copyInfo, err }));
        } else {
          dispatch(smessagesActions.openMessage('Копирование текстуры выполнено.'));
          dispatch(saveBuildingTexturesComplete({ copyInfo }));
        }
      });
  }
}
...
export const actions = {
  ...
  copyBuildingTextureComplete,
  copyBuildingTextureFailed,
  copyBuildingTexture,
  getOriginTextures
};
