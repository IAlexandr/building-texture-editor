    ...
    superagent.put('/api/buildings/' + ParentRegisterNo + '/' + ID)
      ...
      .end((err, res) => {
        if (err) {
          ...
          dispatch(loadBuildingsFailed(newBuilding, err));
        } else {
          dispatch(smessagesActions.openMessage('Сведения по Строению обновлены.'));
          dispatch(loadBuildingsComplete(res.body));
        }
      });
  };
}
