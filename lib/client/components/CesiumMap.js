        ...
        if (Cesium.defined(pickedObjects)) {
          if (pickedObjects[0]) {
            ...
            if (entity.wall) {
              ...
              if (wallId !== this.props.buildings.selectInfo.wallId) {
                this.props.unselect();
                this.props.selectInfo({ registerNo, wallId, ID });
              } else {
                if (ID !== this.props.buildings.selectInfo.ID) {
                  this.props.unselect();
                  this.props.selectInfo({ registerNo, wallId, ID });
                }
              }
    ...
    if (!this.state.viewer) {
      ...
      const viewer = new Cesium.Viewer('cesiumContainer', {
        ...
        imageryProvider: new Cesium.createOpenStreetMapImageryProvider({
          url: 'https://a.tile.openstreetmap.org'// 'http://gwosm.cloudapp.net/osm/'// 'https://a.tile.openstreetmap.org'
        })
      });
      this.setState({
        viewer
