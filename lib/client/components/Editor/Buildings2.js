...
export default class Buildings extends Component {
  ...
  addPoint (x, y, height, label) {
    ...
    Cesium.myViewer.entities.add(point);
  }
  ...
  findSelectedBuilding (props) {
    let resultBuilding;
    props.buildings[props.registerNo].buildings.forEach((building) => {
      if (building.properties.ID === props.selectInfo.ID) {
    ...
    return (
      <div>
        ...
        <FlatButton
          ...
          secondary={true}
          onClick={(() => {
            const building = this.findSelectedBuilding(this.props);
