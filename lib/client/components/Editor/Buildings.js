  ...
  buildBuildingWalls (building) {
    ...
    switch (building.geometry.type) {
      case 'Polygon':
        ...
        if (this.props.location.search === '?view') {
          const hierarchyDegreesArray = [];
          building.geometry.points.forEach((point) => {
            hierarchyDegreesArray.push(point.x);
            hierarchyDegreesArray.push(point.y);
          });
          Cesium.myViewer.entities.add({
            polygon : {
              hierarchy : Cesium.Cartesian3.fromDegreesArray(hierarchyDegreesArray),
              material : new Cesium.ImageMaterialProperty({
                image: '/api/files/roof.png',
                alpha: 0.5
              }),
              height: building.properties['этажность'] * 3,
            }
          });
        }
        break;
